...
module.exports = {
  ...
  description: "Allows you to play Chess in a voice channel.",
  run: async(client, message, args, guild) => {

    if(!args[0]) return message.channel.send(":x: You must provide an ID to the channel where you want to play Chess! " 
    ...
    fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
        ...
        body: JSON.stringify({
            max_age: 7200,
            max_uses: 0,
            target_application_id: config.chess,
            target_type: 2,
            temporary: false,
            validate: null
        }),
    ...
    .then(invite => {
        ...
        let embed = new Discord.MessageEmbed()
            .setTitle("Chess")
            .setDescription(`Click [here](https://discord.gg/${invite.code}) to start playing Chess.\nTo invite a friend, use the following invite: https://discord.gg/${invite.code}`)
            .setColor(client.config.color)
            .setFooter(`Bot developed by ${client.config.owner_tag}`, client.user.displayAvatarURL());
